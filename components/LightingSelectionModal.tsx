...
import { X, Sun, ChevronRight, Circle, Folder, FolderOpen, RotateCcw, MessageSquare, CheckCircle2 } from 'lucide-react';
import { LIGHTING_CATEGORIES } from '../constants/lightingOptions';

interface LightingSelectionModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSelectLighting: (description: string, aspectRatio?: string) => void;
        ...
        <div className="grid grid-cols-3 gap-2">
            {["As-is", "1:1", "16:9", "4:3", "3:4", "9:16"].map(r => (
                <button
                    key={r}
                    onClick={() => onChange(r)}
                    className={`px-2 py-1.5 rounded text-[10px] font-bold border transition-all ${value === r ? 'bg-amber-600 border-amber-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-500'}`}
                >
                    {r}
                </button>
            ))}
...
export const LightingSelectionModal: React.FC<LightingSelectionModalProps> = ({ isOpen, onClose, onSelectLighting, onEditPrompt }) => {
            ...
            <div className="pl-4 border-l border-gray-700 ml-2">
                {Object.entries(nodes).map(([key, value]) => {
                    ...
                    return (
                        <div key={key}>
                            <div 
                                className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer text-sm mb-1 transition-colors ${isSelected ? 'bg-amber-900/40 text-amber-300 font-bold' : 'hover:bg-gray-800 text-gray-300'}`}
                                onClick={() => {
                                    if (isNodeLeaf) handleSelectCategory(newPath);
                                    else toggleExpand(newPathStr);
                                }}
                            >
                                {isNodeLeaf ? <Circle className="w-2 h-2 text-amber-400 fill-current" /> : (isExpanded ? <FolderOpen className="w-4 h-4 text-amber-500" /> : <Folder className="w-4 h-4 text-gray-500" />)}
                                <span>{key}</span>
    ...
    const getOptions = () => {
        ...
        let current: any = LIGHTING_CATEGORIES;
            ...
            <div className="bg-gray-950 rounded-xl shadow-2xl w-[95vw] h-[95vh] flex flex-col overflow-hidden relative border border-gray-800">
                <div className="flex items-center justify-between p-6 border-b border-gray-800 bg-gray-900">
                    <h2 className="text-2xl font-bold flex items-center gap-3 text-gray-100">
                        <Sun className="w-6 h-6 text-amber-500"/> Lighting & Shadows
                    </h2>
                ...
                <div className="flex flex-1 overflow-hidden">
                    <div className="w-1/3 border-r border-gray-800 overflow-y-auto p-4 bg-gray-900 flex flex-col">
                        <div className="space-y-1 flex-1">
                            {renderTree(LIGHTING_CATEGORIES)}
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 bg-gray-950 pb-24">
                        {options ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {(options as string[]).map((option: string) => (
                                    <div key={option} className="flex flex-col gap-2">
                                        <button
                                            onClick={() => { onSelectLighting(option, aspectRatio === "As-is" ? undefined : aspectRatio); onClose(); }}
                                            className="p-5 rounded-xl border border-gray-800 hover:border-amber-500 hover:bg-amber-900/20 hover:shadow-md transition-all text-left group bg-gray-900 w-full"
                                        >
                                            <span className="font-bold text-gray-300 group-hover:text-amber-300 block mb-1">{option.split('(')[0]}</span>
                            ...
                            <div className="flex flex-col items-center justify-center h-full text-gray-600">
                                <Sun className="w-16 h-16 mb-4 opacity-10" />
                                <p>Select a lighting category to adjust scene illumination</p>
                            </div>
                        )}
                    </div>
                </div>
